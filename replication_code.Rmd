---
title: "Upzoning and Residential Transaction Price in Nashville"
output: pdf_document
date: "`r Sys.Date()`"
---
...
## Data Visualization
Producing Log of Price by Treatment figures with Geographic controls. The order of figures is: Yearly trends by 1 Kilometer Control, 3 Kilometer Control, 5 Kilometer Control, 10 Kilometer Control. The final figure uses monthly averages from the 3 Kilometer Control vs treated in the pre-treatment period.
...
Producing Log of Price by  Treatment figures with Matching controls. The order of figures is: Propensity Score Matching (PSM) Control, Generalized Boosted Matching (GBM) Control, Random Forest Matching (RFM) Control. The final figure uses monthly averages fromPropensity Score Matching Control vs treated in the pre-treatment period.
...
Pre-treatment placebo with matched controls (PSM) and Pre-treatment placebo with 3km Control.
...
# Display the summary with stargazer
stargazer(summary_Mean1, type = "text", summary = FALSE, title = "Mean Sale Price by Group and Period",digits=0)
...
# Display the summary with stargazer
stargazer(summary_Combo, type = "text", summary = FALSE, title = "Mean Sale Price Per Square Foot by Group and Period")
...
# Display the summary with stargazer
stargazer(summary_Combo1, type = "text", summary = FALSE, title = "Mean Sale Price Per Square Foot by Group and Period")
...
Balance tables are provided for geographic controls in the order (1km, 3km, 5km, 10km), then matched control balance tables are presented in the order (PSM, GBM, RFM).
...
Difference-in-Differences LnSalePrice and LnSalePriceper sqr foot with Matched  controls.
...
##Temporally segmented regressions
...
Difference-in-Differences by time period (2010-2015, 2016-2023), with geographic controls.
...
##Quantile Difference-in-Differences (Q-DiD)
Running the Q-DiD with geographic controls. The order of tables is: Yearly trends by 1 Kilometer Control, 3 Kilometer Control, 5 Kilometer Control, 10 Kilometer Control. 
...
Running the Q-DiD with Matched sample. The order of tables is: Propensity Score Matching (PSM) Control, Generalized Boosted Matching (GBM) Control, Random Forest Matching (RFM) Control.
