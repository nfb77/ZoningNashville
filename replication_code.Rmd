...
DID1k_avg %>%
  ...
  scale_x_continuous(
    ...
    labels = seq(2000, 2023, 5)
  ) +
  labs(y = "Yearly Average ln(Sale Price)", color = "Treatment") +
...
DID3k_avg_filtered %>%
  ...
  scale_y_continuous(limits = c(10, 15)) +
  labs(y = "Monthly Average ln(Sale Price)", x = "Months Prior to Re-Zoning", color = "Treatment") +
...
psvalue_top_avg %>%
 ggplot(aes(x = Year, y = mean_LnSalePrice, group = CoreBin, color = as.factor(CoreBin))) +
  ...
  labs(y = "Yearly Average ln(Sale Price)", x = "Year",color = "Treatment") +
  geom_vline(xintercept = "2010", linetype = "dashed", color = "black", linewidth = 1) + 
  scale_color_discrete(labels = c("Control", "Treatment")) +  
...
iplot(mod_twfe, ci_level = 0.99, 
      ...
      main = 'ln(Sale Price) 1 km control: Staggered treatment (TWFE)')
...
iplot(mod_twfe, ci_level = 0.99, 
      xlab = 'Time to treatment',
      main = 'ln(Sale Price) 3 km control: Staggered treatment (TWFE)')
...
iplot(mod_twfe, ci_level = 0.99, 
      xlab = 'Time to treatment',
      main = 'ln(Sale Price) 5 km control: Staggered treatment (TWFE)')
...
iplot(mod_twfe, ci_level = 0.99, 
      xlab = 'Time to treatment',
      main = 'ln(Sale Price) 10 km control: Staggered treatment (TWFE)') + theme(panel.background = element_blank())
...
iplot(mod_twfe, ci_level = 0.99, 
      ...
      main = 'Propensity Score Matching, ln(Sale Price) Event Study')
...
iplot(mod_twfe, ci_level = 0.99, 
      xlab = 'Years to treatment',
      main = 'Generalized Boosted Matching, ln(Sale Price) Event Study')
...
iplot(mod_twfe, ci_level = 0.99, 
      xlab = 'Years to treatment',
      main = 'Random Forest Matching, ln(Sale Price) Event Study')
...
DTC_DID_Full <- DTC_DID_Full %>%
  dplyr::select(LnSalePrice, LnPricePerSqrft, CoreBin, TimeBin, SalePrice, Age, FinishArea, PrcntWhite, BAorHigher, BuildPreTrt,QuarterByYear,Year, Quarter) %>%
  na.omit()  
