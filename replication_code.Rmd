...
rf_psvalue_top <- DTC_DIDT1_Control[order(-DTC_DIDT1_Control$rf_psvalue), ][1:200, ]
...
```{r}
#Matching Methods, with complete difference in difference
...
rf_psvalue_top <- DTC_DID_Full_Control[order(-DTC_DID_Full_Control$rf_psvalue), ][1:600, ]
...
```{r}
#########################Estimating the Q-DiD using Matched Data##############################

QDIDpsm <- rq(LnSalePrice ~ CoreBin*TimeBin + Age + FinishArea+ factor(QuarterByYear)+ factor(PropZip), data=psvalue_top, tau=c(.25,.50,.75))
...
percent_change <- (exp(0.192 - 1) * 100 )
percent_change
